...
segments_df['part'] = pd.Categorical(segments_df['part'], ['Unused',
                                                           'CV - Train',
                                                           'CV - Validation',
                                                           'Train',
                                                           'Test',
...
plot += geom_vline(xintercept=40,
                   linetype='dashed',
                   color='#b6dea8',
                   size=1.1)

plot += geom_vline(xintercept=50,
                   linetype='dashed',
                   color='#b6dea8',
                   size=1.1)


plot += geom_text(mapping=aes(x=50, y=7),
                  label='Re-train',
                  color='black',
                  size=10, angle=90)

plot += geom_text(mapping=aes(x=40, y=7),
                  label='Re-train',
                  color='black',
                  size=10, angle=90)

plot += geom_text(mapping=aes(x=25, y=7),
                  label='After Cross-validation',
                  color='black',
                  size=11, angle=0)

plot += geom_text(mapping=aes(x=25, y=1),
...
plot += geom_label(mapping=aes(x=30, y=7),
                   label='Re-train with \n Available Data',
                   #fill='#edf7ea',
                   size=9)

plot.save('cv_nested.pdf', width=10, height=7)
